...
<nav>
    ...
    <div>
        <?php if (is_authenticated()): $me = current_user(); ?>
        <span>Logged in as: <strong><?= e($me['username']) ?></strong><?php if (is_admin()): ?><span class="badge badge-admin">admin</span><?php endif; ?></span>
...
<div class="container">
    <?php $messages = get_flashed_messages(); if ($messages): ?>
    <ul class="flashes">
        <?php foreach ($messages as $message): ?><li><?= e($message) ?></li><?php endforeach; ?>
    </ul>
    <?php endif; ?>
    <?= $content ?? '' ?>
</div>

<?php if (is_authenticated() && empty($_SESSION['remember_me'])): ?>
<dialog id="sessionTimeoutDialog" class="note-dialog">
    <div class="dlg-body">
        <h3>Session about to expire</h3>
        <p>You will be signed out in <strong id="sessionCountdown">&mdash;</strong> if you don't stay active.</p>
